...
foreach ($patients as $patient) {
    ...
    foreach ($patient as $pat_key => $pat_value) {		 
        ...
        if ($pat_key == "patient_id") {
            $patient_id = $pat_value ;
        }		 
		
		if ($pat_key == "age") {
            $pat_value = $this->customlib->get_patient_current_age($patient_id); ;
        }
            ...
            <tr>
                <td style="position: absolute;left:0;">
                    <?php if ($certificate[0]->enable_patient_image == 1) {?>
                        <img style="position: relative; <?php echo "top:" . $certificate[0]->enable_image_height . "px" ?>;" src="<?php echo $this->media_storage->getImageURL($patient->image); ?>" width="100" height="auto">
                    <?php }?>
                </td>
            </tr>
            <tr>
