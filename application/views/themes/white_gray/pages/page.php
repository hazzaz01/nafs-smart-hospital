        ...
        foreach ($page['category_content'] as $page_content_key => $page_content_value) {
            ...
            if ($page_content_key == "events") {
                ...
                <div class="row">
                    ...
                    foreach ($page['category_content'][$page_content_key] as $key => $value) {
                        ...
                        if ($value['feature_image'] == "") {
                            $feature_image = $this->media_storage->getImageURL('uploads/gallery/gallery_default.png');
                        } else {
                        ...
                        <div class="col-md-<?php echo $bootstrapColWidth; ?> col-sm-<?php echo $bootstrapColWidth; ?>">
                            <div class="latestevent">
                                <div class="row">   
                                    <div class="col-md-2">
                                        <div class="eventsdate"><?php echo date('d', strtotime($value['event_start'])); ?> <span><?php echo $this->lang->line(strtolower(date('M', strtotime($value['event_start'])))); ?></span></div>
                                    </div>
                                    <div class="col-md-7">
                                        <h4><?php echo $value['title']; ?></h4>
										<p><?php if($value['event_venue']){ echo $this->lang->line('venue') .': '. $value['event_venue']; } ?></p>    
										<p>									
											<?php 
												echo $this->lang->line('date') .': '; 
												$start = strtotime($value['event_start']);
												$end   = strtotime($value['event_end']);
												if ($value['event_start'] != "" && $value['event_end'] != "") {
													if ($start == $end) {
														echo $this->customlib->YYYYMMDDTodateFormat($value['event_start']);               
													} else {
														echo $this->customlib->YYYYMMDDTodateFormat($value['event_start']). " - " .$this->customlib->YYYYMMDDTodateFormat($value['event_end']);               
													}
												} elseif ($value['event_start'] != "" && $value['event_end'] == "") {
													echo $this->customlib->YYYYMMDDTodateFormat($value['event_start']);            
												}
											?>		
										</p>
                ...
                if (!empty($page['category_content'][$page_content_key])) {
                    foreach ($page['category_content'][$page_content_key] as $key => $value) {
                        ?>
                        <div class="alert-message alert-message-default latestevent">
                            <h4><a href="<?php echo site_url($value['url']); ?>"><?php echo $value['title']; ?></a></h4>
                            <div class="dateevents">01 May</div>
