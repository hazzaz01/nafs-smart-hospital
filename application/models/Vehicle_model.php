...
class Vehicle_model extends MY_Model
{
    ...
    public function addCallAmbulance($data, $transaction)
    {
        if (isset($data['id']) && $data['id'] != '') {
            ...
            if (isset($transaction) && !empty($transaction)) {
                ...
                $this->db->where('patient_id', $transaction['patient_id']);				
    ...
    public function getBillDetails($id)
    {
        ...
        $this->db->select('ambulance_call.*,IFNULL((SELECT SUM(transactions.amount) FROM transactions WHERE transactions.ambulance_call_id = ambulance_call.id  and `transactions`.`patient_id` =`ambulance_call`.`patient_id`), 0) as total_paid,charges.name as charge_name,charge_categories.name as charge_category_name,vehicles.vehicle_no,vehicles.vehicle_model,patients.patient_name as patientname,staff.name,staff.surname,staff.employee_id' . $field_variable);
    ...
    public function getAllambulancecallRecord()
    {
        ...
        $this->datatables
            ...
            ->searchable('ambulance_call.id,ambulance_call.case_reference_id,patients.patient_name,patients.id,vehicles.vehicle_no,vehicles.vehicle_model,ambulance_call.driver,patients.mobileno,patients.address,ambulance_call.date' . $custom_field_column . ',ambulance_call.discount,ambulance_call.discount_percentage,net_amount')
            ->orderable('ambulance_call.id,ambulance_call.case_reference_id,patients.patient_name,patients.id,vehicles.vehicle_no,vehicles.vehicle_model,ambulance_call.driver,patients.mobileno,patients.address,ambulance_call.date' . $custom_field_column . ',ambulance_call.discount,tax_percentage,ambulance_call.discount_percentage,net_amount,paid_amount')	
    ...
    public function getCallAmbulancepat($patient_id)
    {
        ...
        $query = $this->db->select('ambulance_call.*,IFNULL((SELECT sum(transactions.amount) from transactions WHERE transactions.ambulance_call_id=ambulance_call.id and `transactions`.`patient_id` =`ambulance_call`.`patient_id`),0) as `paid_amount`, vehicles.vehicle_no,vehicles.vehicle_model,vehicles.driver_name,vehicles.driver_contact,patients.patient_name as patient,patients.mobileno,patients.address' . $field_variable)
    ...
    public function getBillDetailsAmbulance($id)
    {
        ...
        $field_variable      = (empty($field_var_array)) ? "" : "," . implode(',', $field_var_array);
        $custom_field_column = (empty($custom_field_column_array)) ? "" : "," . implode(',', $custom_field_column_array);
        $query               = $this->db->select('ambulance_call.*,IFNULL((SELECT sum(transactions.amount) from transactions WHERE transactions.ambulance_call_id=ambulance_call.id   and `transactions`.`patient_id` =`ambulance_call`.`patient_id`),0) as `paid_amount`, staff.name as staff_name,staff.surname as staff_surname,staff.employee_id as staff_employee_id,charge_categories.name as charge_category_name,charges.name as charge_name,vehicles.id,vehicles.vehicle_no,vehicles.vehicle_model,vehicles.driver_name,vehicles.driver_contact,patients.patient_name as patient,patients.mobileno,patients.address,staff_roles.role_id as staff_roles_id,' . $field_variable)
            ->join('vehicles', 'vehicles.id = ambulance_call.vehicle_id')
            ->join('patients', 'patients.id = ambulance_call.patient_id')
            ->join('staff', 'staff.id = ambulance_call.generated_by')
    ...
    public function getCallDetails($id)
    {
        ...
        transactions.amount as `payment_amount`')
        ->join('charges', 'charges.id = ambulance_call.charge_id', 'left')
        ->join('charge_categories', 'charge_categories.id = charges.charge_category_id', 'left')
        ->join('transactions', 'transactions.ambulance_call_id = ambulance_call.id and  transactions.patient_id=ambulance_call.patient_id', 'left')
        ->where('ambulance_call.id', $id)
        ->get('ambulance_call');
        return $query->row_array();
    }
    ...
    public function getAmbulanceCallRecord($case_id)
    {
        $this->datatables
            ...
            ->select("ambulance_call.id, vehicles.vehicle_no,ambulance_call.net_amount, ambulance_call.date, ambulance_call.discount, ambulance_call.discount_percentage,ambulance_call.amount,ambulance_call.tax_percentage")
