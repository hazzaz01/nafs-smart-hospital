...
class Pharmacy_model extends MY_Model
{
    ...
    {   
        if ($condition['medicine_category'] != "") {
            $this->datatables->where('`pharmacy`.`medicine_category_id`='.$condition['medicine_category']);
        }
        $this->datatables
            ->select('pharmacy.*,medicine_category.id as medicine_categoryid,medicine_category.medicine_category,(SELECT sum(available_quantity) FROM `medicine_batch_details` WHERE pharmacy_id=pharmacy.id) as `total_qty`,unit.unit_name,pharmacy_company.company_name,medicine_group.group_name')
            ->join('medicine_category', 'pharmacy.medicine_category_id = medicine_category.id', 'left')
            ...
            $this->datatables->where('`pharmacy`.`medicine_category_id`=`medicine_category`.`id`');
    ...
    public function getAllpharmacybillRecord()
    {
        ...
        $this->datatables
            ...
            ->orderable('pharmacy_bill_basic.id, pharmacy_bill_basic.case_reference_id,pharmacy_bill_basic.date, patients.patient_name, generated_by_staff.name as generated_byname, pharmacy_bill_basic.doctor_name' . $custom_field_column . ',pharmacy_bill_basic.discount,pharmacy_bill_basic.tax,
    ...
    public function getAllpharmacybillByCaseIdSql($case_id) 
    {
        ...
        ->searchable('pharmacy_bill_basic.id,pharmacy_bill_basic.date,pharmacy_bill_basic.doctor_name,pharmacy_bill_basic.discount_percentage,pharmacy_bill_basic.net_amount,transaction_add.amt_payment,transaction_refund.amt_refund,pharmacy_bill_basic.case_reference_id,patients.patient_name,pharmacy_bill_basic.total')
        ->orderable('pharmacy_bill_basic.id,pharmacy_bill_basic.date,pharmacy_bill_basic.doctor_name,pharmacy_bill_basic.discount_percentage,pharmacy_bill_basic.net_amount,transaction_add.amt_payment,transaction_refund.amt_refund,pharmacy_bill_basic.case_reference_id,patients.patient_name,pharmacy_bill_basic.total')
        ->sort('pharmacy_bill_basic.id', 'desc')
    ...
    public function getMedicineBatchByID($medicine_batch_detail_id=null,$organisation_id=null)
    {
        $sql   = "SELECT medicine_batch_details.*, `organisations_medicine_charges`.`org_charge`,IFNULL((SELECT SUM(quantity) FROM `pharmacy_bill_detail` WHERE medicine_batch_detail_id=medicine_batch_details.id),0) as used_quantity FROM `medicine_batch_details` 
        ...
        and organisations_medicine_charges.org_id=".$this->db->escape($organisation_id)."
    ...
    public function addtpacharge($data){
        ...
        if (isset($data["id"]) && $data["id"]!="") {
            $this->db->where("id", $data["id"])->update("organisations_medicine_charges", $data);
            $record_id=$data["id"];
        } else {
