...
class Dutyroster_model extends MY_Model
{
    ...
    public function add_roster_assign($data){        
        ...
        if (isset($data['id']) && $data['id']!="") {
            ...
            $message = UPDATE_RECORD_CONSTANT . " On Duty Roster Assign id " . $data['id'];
            $action = "Update";
            $record_id = $data['id'];
            $this->log($message, $record_id, $action);
        } else {
            ...
            $insert_id = $this->db->insert_id();
            $message = INSERT_RECORD_CONSTANT . " On Duty Roster Assign id " . $insert_id;
            $action = "Insert";
            $record_id = $insert_id;
            $this->log($message, $record_id, $action);
        }
    ...
    public function getdutyRoster_assignList(){
        ...
        $this->datatables
            ->select('duty_roster_assign.code,duty_roster_assign.roster_duty_date,duty_roster_assign.floor_id,duty_roster_assign.department_id,duty_roster_assign.staff_id,duty_roster_assign.duty_roster_list_id,duty_roster_shift.shift_name,duty_roster_shift.shift_start,duty_roster_shift.shift_end,duty_roster_shift.shift_hour,
                floor.name as floor_name,department.department_name,staff.name as staff_name,staff.surname as staff_surname,staff.employee_id as staff_employee_id,duty_roster_start_date,duty_roster_end_date,generated_by_staff.name as generated_byname,generated_by_staff.surname as generated_bysurname,generated_by_staff.employee_id as generated_byemployee_id')
            ...
            ->join('staff as generated_by_staff', 'generated_by_staff.id = duty_roster_assign.generated_by', "left")->join('department', 'department.id = duty_roster_assign.department_id','left')  
            ->sort('staff.id', 'asc')
    ...
    $code=   $id;     
        ...
        $message   = DELETE_RECORD_CONSTANT . " On Duty Roster Assign code  " . $code;
        $action = "Delete";
        $record_id = $code;
        $this->log($message, $record_id, $action);
    }  
