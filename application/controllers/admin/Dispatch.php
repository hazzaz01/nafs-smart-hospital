...
class Dispatch extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
                ...
                $this->dispatch_model->image_add('dispatch', $dispatch_id, $file_name);
            }  
    ...
    public function getdispatchdatatable()
    {
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                if ($value->image !== "") {
                    $action1  = "<a href=" . base_url() . 'admin/dispatch/download/' . $value->id . " class='btn btn-default btn-xs pull-right'  data-toggle='tooltip' title='' data-original-title=" . $this->lang->line('download') . "><i class='fa fa-download' aria-hidden='true'></i></a>";
                }
    ...
    public function editdispatch()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
                $file_name = $this->media_storage->fileupload("file",'./uploads/front_office/dispatch_receive/');
                $this->dispatch_model->image_update('dispatch', $id, $file_name);
            } 
    ...
    {
        $receive_data = $this->dispatch_model->dis_rec_data($id,'dispatch'); 
